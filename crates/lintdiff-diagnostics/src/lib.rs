    ...
    pub fn parse_cargo_messages<R: BufRead>(reader: R) -> Result<Vec<Diagnostic>, DiagnosticsParseError> {
        ...
        for (idx, line_res) in reader.lines().enumerate() {
            ...
            let v: Value = serde_json::from_str(&line).map_err(|e| DiagnosticsParseError::InvalidJson {
                line: line_no,
                source: e,
            })?;
            ...
            let msg = v.get("message").ok_or_else(|| DiagnosticsParseError::InvalidShape {
                line: line_no,
            ...
            let rendered = msg.get("rendered").and_then(|x| x.as_str()).map(|s| s.to_string());
            ...
            let spans_val = msg.get("spans").and_then(|x| x.as_array()).cloned().unwrap_or_default();
            ...
            for sp in spans_val {
                ...
                let line_end = sp.get("line_end").and_then(|x| x.as_u64()).unwrap_or(line_start as u64) as u32;
                let col_start = sp.get("column_start").and_then(|x| x.as_u64()).map(|n| n as u32);
                let col_end = sp.get("column_end").and_then(|x| x.as_u64()).map(|n| n as u32);
...
{"reason":"compiler-message","message":{"level":"warning","message":"hi","code":{"code":"clippy::needless_borrow"},"spans":[{"file_name":"src/lib.rs","line_start":3,"line_end":3,"column_start":1,"column_end":2,"is_primary":true}]}}"#;
            ...
            assert_eq!(diags[0].code_raw.as_deref(), Some("clippy::needless_borrow"));
