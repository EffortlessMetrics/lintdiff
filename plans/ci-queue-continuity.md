...
Regex used by the checker: `^\s*Depends-On:\s+#(\d+)\s*$`

- `Closes #123`, `Fixes #123`, and prose references do not create dependencies.
...
This lane does not perform remote mutation. `-ApplyRestack`, `-DryRunRestack`, and `-ConfirmRestack` are read-only and emit:
